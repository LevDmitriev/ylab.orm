...
class YlabElementTable extends ElementTable
{
    ...
    public static function getMap()
    {
        ...
        if ($iIblockId = static::getIblockId()) {
            ...
            $oIblockPropCollection = PropertyTable::getList(['filter' => ['IBLOCK_ID' => $iIblockId]])
                ->fetchCollection();
            foreach ($oIblockPropCollection as $oIblockProp) {
                ...
                if (PropertyTable::TYPE_FILE === $sPropertyType && !$isMultiple) { // Свойство с привязкой к 1 файлу
                    $arResult[] = (new Reference($sPropertyName, YlabFileTable::class,
                        Join::on('this.PROPERTIES.VALUE', 'ref.ID')->where('this.PROPERTIES.IBLOCK_PROPERTY_ID',
                            $oIblockProp->getId())))->configureJoinType('inner');
                } elseif (PropertyTable::TYPE_LIST === $sPropertyType
                          && !$isMultiple) { // Свойство типа список с одиночным выбором
                    $arResult[] = (new Reference($sPropertyName, YlabPropertyEnumTable::class,
                        Join::on('this.PROPERTIES.VALUE_ENUM', 'ref.ID')
                            ->whereNotNull('this.PROPERTIES.VALUE_ENUM')))->configureJoinType('inner');
                } elseif (PropertyTable::TYPE_ELEMENT === $sPropertyType && !$isMultiple) { // Привязка к 1 элементу
                    $arResult[] = (new Reference($sPropertyName, YlabElementTable::class,
                        Join::on('this.PROPERTIES.VALUE', 'ref.ID')
                            ->where('ref.IBLOCK_ID', $oIblockProp->getLinkIblockId())))->configureJoinType('inner');
                } elseif (PropertyTable::TYPE_SECTION === $sPropertyType
                          && !$isMultiple) { // Привязка к 1 разделу инфоблока
                    $arResult[] = (new Reference($sPropertyName, YlabSectionTable::class,
                        Join::on('this.PROPERTIES.VALUE', 'ref.ID')
                            ->where('ref.IBLOCK_ID', $oIblockProp->getLinkIblockId())))->configureJoinType('inner');
                } elseif ($isMultiple) { // Оставшееся, относящиеся к множественному выбору
                    $arResult[] = new OneToMany($sPropertyName, YlabElementPropertyTable::class,
                        'PRIVATE_ELEMENT_BY_PROPERTY_' . $oIblockProp->getCode());
                } else { // Оставшееся, относящееся к одиночному выбору
                    $arResult[] = (new Reference($sPropertyName, YlabElementPropertyTable::class,
                        Join::on('this.ID', 'ref.IBLOCK_ELEMENT_ID')
                            ->where('ref.IBLOCK_PROPERTY_ID', $oIblockProp->getId())))->configureJoinType('inner');
                }
                
            }
        }
        
        $arResult = array_merge(RelationMapperStaticFactory::create(static::getTableName())->getRelations(), $arResult,
            parent::getMap());
