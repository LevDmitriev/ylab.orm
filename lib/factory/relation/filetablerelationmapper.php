...
class FileTableRelationMapper extends RelationMapper
{
    function getRelations()
    {
        ...
        $arResult['IBLOCK_ELEMENT_PROPERTY'] = (new Reference('IBLOCK_ELEMENT_PROPERTY', YlabElementPropertyTable::class,
            Join::on('this.ID', 'ref.VALUE')
                ->where(Query::filter()->logic('or')
                    ->where('ref.IBLOCK_PROPERTY_ID',
                        array_map(function ($id) { return ['ref.IBLOCK_PROPERTY_ID', $id]; },$oIblockPropCollection->getIdList())
                    ))
